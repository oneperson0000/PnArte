...
class Storage {
    ...
    getEntradaById(id) {
        const entradas = this.getEntradas();
        return entradas.find(e => e.id === parseInt(id)) || null;
    }
    ...
    deleteEntrada(id) {
        ...
        // Remove a entrada com ID específico (converte para número para garantir compatibilidade)
        entradas = entradas.filter(e => e.id !== parseInt(id));
    ...
    getSaidaById(id) {
        const saidas = this.getSaidas();
        return saidas.find(s => s.id === parseInt(id)) || null;
    }

    updateSaida(id, novosDados) {
        const saidas = this.getSaidas();
        const index = saidas.findIndex(s => s.id === parseInt(id));
    ...
    deleteSaida(id) {
        ...
        saidas = saidas.filter(s => s.id !== parseInt(id));
        this.setSaidas(saidas);
    ...
    getProdutoById(id) {
        const estoque = this.getEstoque();
        return estoque.find(p => p.id === parseInt(id)) || null;
    }
    ...
    deleteProduto(id) {
        ...
        estoque = estoque.filter(p => p.id !== parseInt(id));
        this.setEstoque(estoque);
